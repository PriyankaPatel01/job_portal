...
const JobListing = () => {
    ...
    useEffect(() => {
        ...
        const newFilteredJobs = jobs.slice().reverse().filter(
            job=>matchesCategory(job) && matchesLoc(job) && matchesTitle(job) && matchesLocation(job)
        )
  ...
  return (
    <div className='container 2xl:px-20 mx-auto flex flex-col lg:flex-row max-lg:space-y-8 py-8'>
            ...
            isSearched && (searchFilter.title !== "" || searchFilter.location !=="") && (
                ...
                <div className='mb-4 text-gray-600'>
                    ...
                    {searchFilter.location && (
                        <span className='inline-flex ml-2 items-center gap-2.5 bg-green-50 border border-green-200 px-4 py-1.5 rounded'>
                            {searchFilter.location}
            ...
            <ul className='space-y-4 text-gray-600'>
                {
                    JobCategories.map((category, index) => (
                        <li key={index} className='flex gap-3 items-center'>
                            <input  className='scale-125 accent-pink-500' type="checkbox" onChange={()=>handleCategoryChange(category)} checked = {selectedCategories.includes(category)}/>
                            {category}
                        </li>
                    ))
                }
            ...
            <ul className='space-y-4 text-gray-600'>
                {
                    JobLocations.map((location, index) => (
                        <li key={index} className='flex gap-3 items-center'>
                            <input className='scale-125 accent-pink-500' type="checkbox" onChange={()=>handleLocationChange(location)} checked = {selectedLocations.includes(location)}/>
                            {location}
                        </li>
                    ))
                }
      ...
      <section className='w-full lg:w-3/4 text-gray-800 max-lg:px-4 '>
        <h3 className='font-medium py-2 text-3xl' id='job-list'>Latest Jobs</h3>
